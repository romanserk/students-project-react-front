...
const SearchResults = React.lazy(() =>
  import("./components/landing/searchResults")
);
const Login = React.lazy(() => import("./components/user/Login"));
const Register = React.lazy(() => import("./components/user/Register"));
const addProjects = React.lazy(() =>
  import("./components/projects/addProject")
);
...
const App = props => {
  ...
  const countVisitor = () => {
    ...
    const date = `date: D${currD.getDate()}-M${currD.getMonth() +
      1}-Y${currD.getFullYear()} time: ${currD.getHours()}:${currD.getMinutes()}:${currD.getSeconds()} timeZone: ${
    ...
    axios(`https://ipapi.co/json/`).then(response => {
      ...
      axios
        .get(`${process.env.REACT_APP_API_FIREBASE_DATABASE}/visitors.json`)
        .then(response => {
          const data = response.data;
          Object.keys(data).forEach(val => {
            if (
              data[val].ip === visitor.ip &&
              data[val].website === visitor.website
            ) {
              exists = true;
            }
          });
...
const mapDispatchToProps = dispatch => {
  return {
    setLoggedIn: isLogged =>
      dispatch({ type: actionType.SET_LOGGED_IN, isLogged: isLogged }),
    setUserData: userData =>
      dispatch({ type: actionType.SET_USER_DATA, userData: userData })
  };
};
