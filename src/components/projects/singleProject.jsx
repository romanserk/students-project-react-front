...
const SingleProject = (props) => {
    ...
    const joinHandler = async () => {
        await joinProject(props.userData.ID, props.singleProject.ID)
            .then(() => {
    ...
    const leaveHandler = async () => {
        await leaveProject(props.userData.ID, props.singleProject.ID)
            .then(() => {
    ...
    const getProject = async (name) => {
        ...
        await getSingleProjectFromServer(name, props.userData.ID)
            .then(project => {
            ...
            <Container>
                < Card className="mt-5 p-3">
                    <Card.Body>
                        ...
                        <Card.Text className="mt-4">{props.singleProject.description}</Card.Text>
                        <Row className="mb-2 mt-5">
                            <Col sm={2}>
                                <Card.Text>Github repository </Card.Text>
                            </Col>
                            <Col sm={10}>
                                <a href={props.singleProject.git_link} target="_blanck">{props.singleProject.git_link}</a>
                            </Col>
                        </Row>
                        <Row className="mb-2">
                            <Col sm={2}>
                            ...
                            <Col sm={10}>
                                <ul className="p-0 m-0 list-inline">
                                    {props.singleProject.tool_ps && props.singleProject.tool_ps.map((tool) => {
                                        return (
                        ...
                        <Row className="mb-2">
                            ...
                            <Col sm={10}>
                                <ul className="p-0 m-0 list-inline">
                                    {props.singleProject.participants && props.singleProject.participants.map((participant) => {
                                        return (
                                            <li key={`${participant.id}${participant.user_name}`} className="mr-5 list-inline-item">
                                                <Link
                                                    to={{
                        ...
                        <ProjectButton joinHandler={joinHandler} leaveHandler={leaveHandler} deleteHandler={deleteHandler} />
...
const mapDispatchToProps = dispatch => {
    return {
        setSingleProject: (singleProject) => dispatch({ type: actionType.SET_SINGLE_PROJECT, singleProject: singleProject }),
        setProjectParticipant: (bool) => dispatch({ type: actionType.SET_PROJECT_PARTICIPANT, participant: bool }),
    };
}
